/*
  ...
  Shooter
  Left & Right to control the gun
  Up = fire button
  Down is start new game
*/
#include <Bounce2.h>
...
int   birdPosX = -1; // targer position
int   bulletPosX = 0; // bullet X position
int   bulletPosY = ROWS-1; // bullet Y position
int   gunPosX = 2; // gun X position
bool  fire; // fire signal
...
void setup() {
  ...
  Serial.begin(19200);
  Serial.println("Charlieplexing shooter game");
  Serial.println("");     
  ...
  if (play) {
    ...
    if ((currentMillis - previousBirdWaitMillis >= birdWait) || birdFlies) {
      ...
      if (currentMillis - previousBirdMillis >= birdSpeed) {
        ...
        if ((birdPosX > COLOMNS-1)||(birdPosX < 0)){
          birdDir = -1;
          if (birdPosX<0) birdDir=1;
    ...
    if ((birdPosX==bulletPosX) && (bulletPosY==0)&& countOnce) {
      scoreCounter++;
      Serial.print ("Score; ");
      Serial.println (scoreCounter);
      countOnce=false;
...
//     Serial.println("no button pressed");     
  ...
  //check colomn borders
  if (gunPosX > COLOMNS-1) gunPosX=COLOMNS-1;
  if (gunPosX < 0) gunPosX=0;

/*
  if (sema_once){
    Serial.print ("gunPos: ");
    Serial.println(gunPosX);
    Serial.println();
    sema_once=false;     
  } 
*/
