...
/* 
 * This data is horizontally flipped. 
 * The bit on the right is the left bit.
 */
...
void updateFrameBuffer() {
  ...
  // write your code here to manipulate the frame buffer.
}
...
void drawFrame(uint32_t repeat) {
  for (uint32_t i = 0; i < repeat; i++) {
    ...
    for (uint8_t y = 0; y < AMOUNT_OF_LEDS_VERTICAL; y++) {
      ...
      for (uint8_t x = AMOUNT_OF_LEDS_HORIZONTAL - 1; x >= 0; x--) {
        // is on
